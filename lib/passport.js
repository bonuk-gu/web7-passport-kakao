module.exports = function (app) {
  ...
  // 로그인 성공시 passport.use에서 done의 두 번째 인자가 serializeUser의 콜백 함수의 첫번째 인자로
        ...
        dbconn.query(
          ...
          (err, row, fields) => {
            if (err) {
              ...
              if (row[0]) {
                dbconn.query(
                  `update users set google_id='${profile.id}' where email='${email}'`,
                  (err, row, fields) => {
                    if (err) {
                      console.log(err);
                    }
                ...
                dbconn.query(
                  `insert into users (email, pwd, displayname, google_id) values ('${profile.emails[0].value}', '1111', '${profile.displayName}', '${profile.id}')`,
                  (err, row, fields) => {
                    if (err) {
                      console.log(err);
                    }
        ...
        dbconn.query(
          ...
          (err, row, fields) => {
            if (err) {
              ...
              if (row[0]) {
                dbconn.query(
                  `update users set kakao_id='${profile.id}' where email='${email}'`,
                  (err, row, fields) => {
                    if (err) {
                      console.log(err);
                    }
                ...
                dbconn.query(
                  `insert into users (email, pwd, displayname, kakao_id) values ('${email}', '1111', '${profile.displayName}', '${profile.id}')`,
                  (err, row, fields) => {
                    if (err) {
                      console.log(err);
                    }
        ...
        dbconn.query(
          ...
          (err, row, fields) => {
            if (err) {
              ...
              if (row[0]) {
                dbconn.query(
                  `update users set naver_id='${profile.id}' where email='${email}'`,
                  (err, row, fields) => {
                    if (err) {
                      console.log(err);
                    }
                ...
                dbconn.query(
                  `insert into users (email, pwd, displayname, naver_id) values ('${email}', '1111', '${profile.displayName}', '${profile.id}')`,
                  (err, row, fields) => {
                    if (err) {
                      console.log(err);
                    }
